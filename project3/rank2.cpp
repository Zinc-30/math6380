...
/*
 ...
 innerW, the adaptive parameters for the inner product
 */
real *drugW, *drugb, *affineW, *affineb, *innerW;
// gradients of parameters
real *grad_drugW, *grad_drugb, *grad_affineW, *grad_affineb, *grad_innerW;
struct DataPair{
    int sample_id;
    int cell_id;
    int drug1_id;
...
void ClearGradients()
{
    ...
    for (int i = 0; i < gene_size; i++) grad_innerW[i] = 0;
}
...
void UpdateParameters()
{
    ...
    for (int i = 0; i < gene_size; i++) innerW[i] += -lr * grad_innerW[i];
}
...
Result TrainPredictOneData(DataPair cur_pair, bool train)
{
    ...
    for (int k = 0; k < tmp_gene_num; k++){
        ...
        s += innerW[gene_id] * (drugW[tmp_drug1_id * gene_size + gene_id] - drugW[tmp_drug2_id * gene_size + gene_id]);
    }
    ...
    if (train){
        ...
        for (int k = 0; k < tmp_gene_num; k++){
            int gene_id = cell_feature[tmp_cell_id][k];
            grad_drugW[tmp_drug1_id * gene_size + gene_id] += ds * innerW[gene_id];
            grad_drugW[tmp_drug2_id * gene_size + gene_id] += -ds * innerW[gene_id];
            grad_innerW[gene_id] += ds * (drugW[tmp_drug1_id * gene_size + gene_id] - drugW[tmp_drug2_id * gene_size + gene_id]);
        }
    }
    return result;
}
...
void InitParameters()
{
    ...
    innerW = (real *)calloc(gene_size, sizeof(real));
    ...
    grad_innerW = (real *)calloc(gene_size, sizeof(real));
}
